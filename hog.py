...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        if who:
            ...
            score = score0
        amount = score - previous_score
        if amount > previous_high:
            if amount == 1:
               print("1 point! That's the biggest gain yet for Player 1")
               return announce_highest(who, 1, amount)
            else:
                print(amount, "points! That's the biggest gain yet for", player)
                return announce_highest(who, amount, score)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    while k <= 9:
        ...
        value1 = averaged_dice(k, dice)
        value2 = averaged_dice(k + 1, dice)
        if value2 > max:
            max = value2
            num = k + 1
        if value1 > max:
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    if is_swap(score, opponent_score) and opponent_score > score and opponent_score - score >= margin:
        return 0
    result = bacon_strategy(score, opponent_score, margin, num_rolls)
    return result 
    # END PROBLEM 11
